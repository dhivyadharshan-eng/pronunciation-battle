 ...
 * Fully compatible with file:// protocol, Chrome, Edge, and HTTP local servers.
...
(function () {
  ...
  // ==========================================
  // 6. HOST DASHBOARD COMPONENT (WITH POLYGLOT LOGO & NEON PINK BUTTONS)
  // ==========================================
  class HostDashboard {
        ...
        <div class="bg-[#040c24] border border-[#8A2BE2]/50 rounded-2xl p-6 shadow-2xl backdrop-blur-xl text-slate-100 max-w-6xl mx-auto my-6">
          <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-6 border-b border-[#8A2BE2]/30">
            <div>
              <div class="flex items-center gap-3">
                <img src="images/logo.jpg" 
                     onerror="this.onerror=null; this.src='file:///C:/Users/acer/.gemini/antigravity/brain/f720631e-eb29-49d2-99f2-d998d6f1ccc4/.user_uploaded/media_1789553301837.jpg';"
                     alt="Polyglot Logo" 
                     class="w-10 h-10 polyglot-logo-img object-cover">
                <div>
            ...
            <div class="flex items-center gap-3 w-full md:w-auto">
              ...
              <button id="btnConnectHost" class="btn-neon-pink font-bold px-4 py-1.5 rounded-lg text-sm transition flex items-center gap-2">
            ...
            <div>
              <div class="flex items-center justify-between mb-2">
                <h4 class="font-bold text-slate-100 text-lg flex items-center gap-2">
                  <i class="fas fa-shield-alt text-[#FF00EF] text-sm"></i> ${team.teamName}
                </h4>
                ${statusBadge}
              </div>
              <div class="text-xs text-slate-400 space-y-1 mb-3">
                <p><i class="fas fa-users text-purple-400 mr-1"></i> ${team.members.join(', ')}</p>
              ...
              ${lastRound ? `
                <div class="bg-[#0b163d] rounded-lg p-2.5 border border-[#8A2BE2]/30 text-xs space-y-1">
                  <div class="flex justify-between font-mono">
                    ...
                    <span class="font-bold ${lastRound.accuracy >= 80 ? 'text-emerald-400' : 'text-[#FF00EF]'}">${lastRound.accuracy}%</span>
                  </div>
  ...
  class PronunciationBattleApp {
    ...
    async handleRegistration(event) {
      ...
      try {
        ...
        this.showRegError("Error during registration. Switching to battle arena...");
        this.showView('battle');
      } finally {
        this.resetRegButton();
      }
    ...
    loadRoundState() {
      ...
      const currentMember = this.session.members[roundIndex % 3];
    ...
    resetSpeechControlsUI() {
      if (this.txtLiveTranscript) {
        this.txtLiveTranscript.textContent = 'Press the logo button and speak clearly into your device...';
      ...
      if (this.micStatusText) this.micStatusText.textContent = 'Tap Logo Microphone to Speak';
      if (this.btnMicRecord) {
        this.btnMicRecord.className = 'w-20 h-20 rounded-full btn-neon-pink text-3xl font-bold flex items-center justify-center shadow-2xl';
      }
    }
    ...
    handleSpeechStatusChange(status) {
      if (status === 'listening') {
        ...
        if (this.micStatusText) {
          this.micStatusText.textContent = this.currentAttempt.transcript ? 'Speech Captured' : 'Tap Logo Microphone to Speak';
        }
      }
    }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => { window.app = new PronunciationBattleApp(); });
  } else {
